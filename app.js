...
function findCachedTeacherByPin(passcode, expectedStream) {
    const want = String(passcode || '').trim().toLowerCase().replace(/[\s\.\-_]/g, '');
    if (!want) return null;
    const wantStream = String(expectedStream || '').trim().toUpperCase().replace('BCOM', 'BCM');
    const list = loadCachedTeachers();
    let wrongStream = null;
    for (let i = 0; i < list.length; i++) {
        const t = list[i];
        if (!t || t.disabled) continue;
        const pin = String(t.pin || '').trim().toLowerCase().replace(/[\s\.\-_]/g, '');
        if (!pin || pin !== want) continue;
        const tStream = String(t.stream || 'BCA').trim().toUpperCase().replace('BCOM', 'BCM') || 'BCA';
        if (!wantStream || tStream === wantStream) return t;
        if (!wrongStream) wrongStream = t;
    }
    if (wrongStream) {
        return { _wrongStream: true, stream: String(wrongStream.stream || 'BCA').toUpperCase().replace('BCOM', 'BCM'), name: wrongStream.name || '' };
    }
    return null;
}
    ...
    return new Promise((resolve) => {
        ...
        const tryLocalVerification = () => {
            try {
                ...
                // Personal teacher PIN (cached) — must match selected stream
                const cachedTeacher = findCachedTeacherByPin(pass, stream);
                if (cachedTeacher && cachedTeacher._wrongStream) {
                    return {
                        ok: false,
                        offline: true,
                        wrongStream: true,
                        message: 'PIN is for ' + (streamLabels[cachedTeacher.stream] || cachedTeacher.stream) +
                            ' — select that stream card.'
                    };
                }
                if (cachedTeacher) {
                    return {
        ...
        window[cbName] = function (data) {
            ...
            if (data && data.result === 'success') {
                ...
                // Stream PINs and personal teacher PINs both stay locked to the selected card
                if (role !== 'ADMIN' && (data.matchedOtherStream || serverStream !== stream)) {
                    settle({
                        ok: false,
                        wrongStream: true,
                        message: data.message || ('PIN is for ' + (serverStream || 'another stream') + ' — select that stream card.')
                    });
                    return;
                }
...
function inferRollPrefixFromList(rolls) {
    const list = (rolls || []).map(r => String(r).trim()).filter(r => {
        if (!r) return false;
        // Need enough trailing digits to leave a 2–3 digit short suffix
    ...
    // Keep at least 2 digit characters available for short entry
    const m = common.match(/^(.*?)(\d*)$/);
    if (!m) return '';
    let letterPart = m[1] || '';
    let digitPart = m[2] || '';
    // Prefer leaving 3 digits for short entry when possible
    ...
    // Prefix should leave a short suffix of 2–3 on typical rolls
    const shortLen = list[0].length - prefix.length;
    if (shortLen < 2 || shortLen > 4) {
        // Fallback: drop last 3 digits of a pure numeric / letter+digit roll
        const sample = list[0];
...
function expandShortRollNumbers(rollInput, prefix) {
    ...
    const out = items.map((token) => {
        ...
        if (!t) return t;
        // Already letter-prefixed code (S0105) — never auto-expand
        if (/^[A-Za-z]+\d+$/.test(t)) return t;
        // Pure digits
        if (/^\d+$/.test(t)) {
            if (!p) return t;
            if (t.length >= 4) return t; // already full-looking
            // 1–3 digits → prefix + short
            if (/^\d+$/.test(p)) return p + t;
            // Prefix like S0 / S01
            return p + t;
        }
        return t;
...
function prepareRollPrefixForEdit(item) {
    ...
    // Focus roll box so teacher can type short digits immediately
    try {
        if (rollNumbersInput) {
            setTimeout(() => {
...
function updateRollExpandPreview(forModal) {
    ...
    const changed = expanded !== normalizeRollNumbers(raw).join(', ');
    if (changed && expanded) {
...
function applyRollPrefixExpansion(rollRaw, yearVal, sectionVal, options) {
    ...
    // Learn / refresh prefix from full rolls when possible
    if (rolls.length > 0) {
...
async function submitData(dateVal, rollNumbersRaw, yearVal, sectionVal, subjectVal, slotVal, btnElem, textElem, spinnerElem, opts) {
    ...
    try {
        ...
        if (targetUrl) {
            // Paste / multi-slot labs: await each POST so later slots (e.g. 7) are not dropped by races
            if (opts.awaitSheet) {
                try {
                    ...
                    console.warn('Sheet POST failed; marking Pending Sync:', error);
                    try {
                        ...
                        if (!opts.silent) showSuccessToast(pending);
                    } catch (e2) {}
                    return { status: 'offline' };
                }
            }
...
async function handleMultiSlotSubmit(dateVal, masterRollRaw, yearVal, sectionVal, subjectVal, startSlotVal, durationVal, breakdownEl, btnElem, textElem, spinnerElem) {
    ...
    for (let slotNum = startSlot; slotNum <= endSlot; slotNum++) {
        ...
        // Expand each slot's shorts with the same section prefix
    ...
    const multiOpts = { awaitSheet: true, skipRefresh: true };
    for (let slotNum = startSlot; slotNum <= endSlot; slotNum++) {
        ...
        try {
            ...
            // Brief gap so Apps Script can finish each Raw row before the next slot
            if (slotNum < endSlot) {
    ...
    for (let i = 0; i < history.length; i++) {
        if (history[i].offline) {
            ...
            try {
                ...
                // Only clear Pending Sync when sheet confirms the row (no-cors alone is not enough)
                if (verify.verified) {
                    history[i].offline = false;
                    history[i].syncNote = '';
                    syncedCount++;
                } else if (posted) {
                    console.warn('Offline sync POST sent but not yet verified on sheet:', item.subject, item.slot);
                }
            } catch (err) {
...
function applyServerHistoryMerge(stream, serverEntries, replaceMode, dateSet, opts) {
    ...
    history.forEach(item => {
        ...
        if (replaceMode === 'stream') {
            return; // sheet is source of truth for this stream
        }
        if (replaceMode === 'dates' && dates && dates.has(normalizeHistoryDate(item.date))) {
            return;
        }
        byKey.set(k, item);
    });
...
/** After writes: refresh today cheaply, or full stream pull for past dates / labs / paste. */
...
function populateHistorySubjectFilter() {
    ...
    if (currentHistoryTabMode === 'ALL') {
        base = readAllHistory().filter(item => isStreamMatch(item.stream, currentDept));
    } else {
...
function collectBulkSlotConflicts(generatedItems) {
    ...
    (generatedItems || []).forEach(item => {
        ...
        const occupant = history.find(h => {
            if (!isStreamMatch(h.stream || stream, stream)) return false;
            ...
            const overlap = isSectionOverlap(h.section || 'A', item.section || 'A', stream);
            const combLock = isCombinedVsSpecificSectionConflict(h.section, item.section);
            if (!overlap && !combLock) return false;
            return true;
        });
        if (!occupant) return;
...
function alertBulkSlotConflicts(conflicts, newSubject, slotVal) {
    const lines = conflicts.slice(0, 8).map(c =>
        '• ' + c.date + ' — ' + (c.existingSubject || 'Subject') + ' (' + sectionDisplayLabel(c.existingSection) + ')'
    );
...
function paperPasteParseDataLine(line, defaultSlot) {
    ...
    let slot = '';
    // "Slot 7" or bare "7" after the date (labs often type: 23.7.26 7 12,15)
...
async function loadPaperPasteToSheet() {
    ...
    // Await each sheet write so later lab slots (incl. 7–8) are not lost to Apps Script races
    const submitOpts = { skipReset: true, silent: true, skipRefresh: true, awaitSheet: true };
    ...
    // Past-class paste: pull full Raw Data so phone matches sheet (not today-only)
    if (typeof fetchFullSheetHistory === 'function') {
        setTimeout(() => fetchFullSheetHistory(), 700);
    } else if (typeof fetchTodayServerHistory === 'function') {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    try {
        ...
        window.addEventListener('resize', () => {
            // Keyboard open/close fires resize — skip while typing or keyboard stays up
    ...
    [dateInput, yearSelect, sectionSelect, subjectInput, slotSelect, rollNumbersInput].forEach(elem => {
        if (elem) {
            const run = () => {
                ...
                // Do not rebuild slot list while typing rolls (causes field/keyboard flash)
                if (elem !== rollNumbersInput) refreshAllSlotDropdowns();
                updateModalDoubleEntryCheck();
            };
    ...
    [directDateInput, directYearSelect, directSectionSelect, directSubjectInput, directSlotSelect, directRollInput].forEach(elem => {
        if (elem) {
            const run = () => {
                ...
                if (elem !== directRollInput) refreshAllSlotDropdowns();
                updateDirectDoubleEntryCheck();
                try { updateMarkAbsenteesStepUI(); } catch (e) {}
            };
            elem.addEventListener('change', run);
...
(function checkAppCacheVersion() {
    const APP_VER = 'v95_stream_teachers';
    const OWN_CACHE_PREFIX = 'mgm-absentee-informer';
...
function initTeachersManager() {
    ...
    const renderTeachers = () => {
        ...
        syncStreamLabel();
        if (!teachers.length) {
            listEl.innerHTML = '<p style="font-size: 0.8rem; color: var(--text-muted);">No teachers for ' +
                escapeHTML(streamNice(activeStream())) + ' yet. Add a name, set a PIN, then Save.</p>';
            return;
        }
    ...
    if (addBtn) {
        addBtn.addEventListener('click', () => {
            ...
            let pin = '';
            for (let i = 0; i < 6; i++) pin += alphabet.charAt(Math.floor(Math.random() * alphabet.length));
            teachers.push({ id: '', name: name, pin: pin, disabled: false, stream: activeStream() });
            if (nameInput) nameInput.value = '';
            renderTeachers();
            setStatus('Added ' + name + ' for ' + streamNice(activeStream()) + ' — tap Save to Server.');
        });
    }
    ...
    if (saveBtn) {
        saveBtn.addEventListener('click', () => {
            ...
            setStatus('Saving ' + streamNice(stream) + ' teachers…');
            ...
            const attemptSave = (pinIdx, round) => {
                ...
                const roundNum = round || 0;
                setStatus(roundNum === 0
                    ? ('Saving ' + streamNice(stream) + ' teachers…')
                    : ('Still saving… (retry ' + (roundNum + 1) + ')'));

                const cbName = 'mgmTeachersSave_' + Date.now() + '_' + pinIdx + '_' + roundNum;
                let settled = false;
                ...
                window[cbName] = function (data) {
                    ...
                    if (roundNum < 2 && (!data || data.result !== 'success')) {
                        attemptSave(pinIdx, roundNum + 1);
                        return;
                    }
                    if (pinIdx + 1 < tryAuthPins.length) {
                        attemptSave(pinIdx + 1, 0);
                        return;
                    }
                    finishSaveUi();
...
function isSubjectMatching(subj1, subj2) {
    ...
    if (String(subj2).trim().toUpperCase() === 'ALL' || String(subj1).trim().toUpperCase() === 'ALL') return true;
    const s1 = String(subj1).toLowerCase().replace(/\s+/g, ' ').trim();
    const s2 = String(subj2).toLowerCase().replace(/\s+/g, ' ').trim();
    if (s1 === s2) return true;
    const base1 = extractSubjNameAndSection(s1).name.trim().toLowerCase().replace(/\s+/g, ' ');
    const base2 = extractSubjNameAndSection(s2).name.trim().toLowerCase().replace(/\s+/g, ' ');
    // Exact paper only — "DBMS" must not count "DBMS Lab" (all streams).
    return base1 === base2;
}
